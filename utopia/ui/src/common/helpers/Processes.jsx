...
class ProcessLog extends React.Component {
    ...
    showLogs = () => {
        ...
        api.processes.process
            .log(process.group, processUniqueName)
            .then(data => {
                console.log(data);
                this.setState({
                    logs: data.logs
...
const Process = ({node, process, refresh}) => {
    const handleProcess = action => {
        ...
        const processUniqueName = `${process.group}:${process.name}`;
        api.processes.process[action](process.group, processUniqueName).then(data => {
            console.log(data);
            refresh();
        });
    };
...
class Processes extends React.Component {
    ...
    handleAllProcess = action => {
        ...
        const meta = this.props.node.general.meta;
        console.log(meta);
        if (meta === "group") {
            console.log("allGroupProcesses");
            api.nodes.allGroupProcesses[action](nodeName).then(() => {
