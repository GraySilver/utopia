...
const nodes = {
  get: async () => {
    try {
      ...
      console.log(error);
      alert("获取Superviosr信息失败，请检测后端返回信息。");
      return [];
    }
  },
  getNode: nodeName => {
    ...
    allGroupProcesses: {
    start: nodeName => {
      return getRequest(`${API_PREFIX}/nodes/${nodeName}/all-group-processes/start/`);
    },
    stop: nodeName => {
      return getRequest(`${API_PREFIX}/nodes/${nodeName}/all-group-processes/stop/`);
    },
    restart: nodeName => {
      return getRequest(
        `${API_PREFIX}/nodes/${nodeName}/all-group-processes/restart/`
      );
    }
  },
  allProcesses: {
